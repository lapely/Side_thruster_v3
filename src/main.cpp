...
void logData(){
  Serial.print(millis());
  Serial.print(",");
  Serial.print((float)position.distance_mm/1000-setP);
  Serial.print(",");
  Serial.print(pid);
  Serial.print(",");
  Serial.print(pwm[0]);
  ...
  if(digitalRead(killSwitchPin) == 0 || killSwitchPin == 0){
    if(mainStatus == 1){ // Trigger loop at time interrupt
      ...
      if (position.range_status == 0){ // Make sure the position sensor see the drone
        ...
        pid = PID(lowpass((double)position.distance_mm, fcut, filteredDataOld, dt)); // Calculate PID
